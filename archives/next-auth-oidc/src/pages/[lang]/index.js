    ...
    <>
      ...
      <ul>
        ...
        <li>
          ...
          <br />
          In this example, the settings indicates that the authentication is done by an external OIDC provider (AWS
          Cognito) via Authorization code
        </li>
        ...
        <li>
          If the user is successfully authenticated, Cognito redirects the user to our website via /login/callback with
          an authorization code (the path can be specified in settings.js)
        </li>
        <li>
          The /login/callback sends a request to the back end server with the authorization code (backend is defined in
          src/pages/api/...)
        </li>
        <li>
          The back end uses the authorization code to receive an id token / access token / refresh token from AWS
          Cognito and stores all these tokens in a secure "httpOnly" cookie
          <br />
        </li>
        <li>
          The /login/callback does a second query to the backend to retrieve the content of the id token (via /userinfo
          endpoint)
          <br />
          Indeed, the /login/callback can't have access to the content of the cookie as it's an secure httpOnly one
        </li>
      </ul>
      <ul>
