...
> extends Service<S> implements Collection<T, M> {
  ...
  query(query: Query): void {
    let resetData = false;
    if (['filter', 'search', 'sort', 'sortBy', 'fields'].filter(value => Object.keys(query).includes(value)).length > 0) {
      resetData = true;
    }
  ...
  protected _addFilter(query: Query, filterOrCriteria: QueryFilterOrCriteria, parentFilterId: QueryFilterId = rootFilterId): void {
