  ...
  try {
    ...
    if (isOauth(idp)) {
      ...
      if (typeof idp.authorizeEndpoint === 'function') {
        ...
        const url = yield idp.authorizeEndpoint(params, idpContext);
      ...
      // this function the task of building the URL of the external login page
      const url = yield idp.externalLoginEndpoint(idpContext);
      window.location.href = `${absoluteUrl(
        url,
        get(settings, 'server.baseUrl')
      )}`;
...
function* externalLoginCallback({ idpName, onError, onSuccess }, context) {
  ...
  try {
    ...
    if (isOauth(idp)) {
      ...
      if (responseType === 'code') {
        if (typeof idp.tokenEndpoint === 'function') {
          ...
          token = yield idp.tokenEndpoint('authorization_code', idpContext);
        } else {
    ...
    if (typeof callback === 'function') {
      ...
      [token, securityContext] = yield callback(response, idpContext);
    }
...
function* formLogin(
  ...
  try {
    ...
    if (typeof idp.callback === 'function') {
      ...
      // the response
      [token, securityContext] = yield idp.callback(response, idpContext);
  ...
  try {
    ...
    if (isExternal(idp)) {
      if (typeof idp.externalLogoutEndpoint === 'function') {
        ...
        // page
        const url = yield idp.externalLogoutEndpoint(idpContext);
        window.location.href = `${absoluteUrl(
          url,
          get(settings, 'server.baseUrl')
        )}`;
      ...
      if (typeof idp.logoutEndpoint === 'function') {
        ...
        yield idp.logoutEndpoint(idpContext);
