  ...
  fetchSecurityContext: leading(function* (
    ...
    try {
      ...
      if (!idp) {
        throw new HTTPError(
          500,
          `Could not find the configuration for IDP ${idpName} in the settings`
        );
      }
      ...
      if (typeof userinfoEndpoint === 'function') {
        ...
        securityContext = yield asyncGet(
    ...
    try {
      ...
      if (isNull(result)) {
        ...
        const expires_at = parseInt(
          yield getItem('onekijs.expires_at', storage)
        );
    ...
    try {
      if (idp.validate) {
        if (!idp.jwksEndpoint) {
          throw new HTTPError(
            500,
            'A jwksEndpoint is required to validate tokens'
          );
        }
        if (token.id_token) {
