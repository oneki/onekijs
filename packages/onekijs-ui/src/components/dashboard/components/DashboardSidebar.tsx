...
import { DashboardArea, DashboardPanel, DashboardPanelProps, DashboardSidebarProps, DashboardSize } from '../typings';
...
const isCollapsed = (size: DashboardSize, collapse?: 'auto' | boolean, floating?: 'auto' | boolean) => {
...
export const DashboardSidebarComponent: React.FC<DashboardSidebarProps> = ({ area, panel, children, ...divProps }) => {
...
export const DashboardSidebar = styled(DashboardSidebarComponent)`
  ...
  height: 100%;
  width: ${(props) => getWidth(props.area, 'small', props.panel)};
...
const sidebar = (area: DashboardArea): React.FC<DashboardPanelProps> => {
  const Component = () => {
    const service = useDashboardContext();
    const panel = service.getPanel(area);
    if (!panel) {
      return null;
    }
    return <DashboardSidebar panel={panel} area={area} />;
  };
...
DashboardLeft.displayName = 'DashboardRight';
