...
export const useElementScroll = ({ parentRef, horizontal, useObserver, initialRect }) => {
...
export const useWindowScroll = ({ windowRef, parentRef, horizontal, useWindowObserver, initialRect }) => {
  const [scrollOffset, setScrollOffset] = React.useState(0);
  const [element, setElement] = React.useState(windowRef.current);
  ...
  const scrollToFn = React.useCallback(
    (offset, reason) => {
      if (windowRef.current) {
        const delta = ['ToIndex', 'SizeChanged'].includes(reason) ? parentOffsetRef.current : 0;
...
export function useVirtual({
  ...
  const scrollTo = React.useCallback(
    (offset, reason) => {
      const resolvedScrollToFn = scrollToFn || ((offset) => defaultScrollToFn(offset, reason));
  ...
  const measurements = React.useMemo(() => {
    const min =
      pendingMeasuredCacheIndexesRef.current.length > 0 ? Math.min(...pendingMeasuredCacheIndexesRef.current) : 0;
